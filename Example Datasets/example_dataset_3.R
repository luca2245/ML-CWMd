...
# Fixed effects
beta1 <- c(-0.52089065,  0.08335639, -1.30575016,  0.22371244,  3.33094751,  2.75188423) 

beta2 <- c(-0.06715653,  0.79119887, -0.45963063,  0.25307799, -3.88824880, -0.63264122)

beta3 <- c(-0.4216015, -0.3076446, -1.3255474, -0.6001705, -4.1797789,  1.8872690)
...
result1 <- cbind(rmultinom(w[1]*n_obs, size = 1, prob = probs2),
...
data <- data %>%
  ...
  dplyr::mutate(lin_pred = if_else(latent == 1,
                                   group_intercepts_1[level] + beta1[1]*x1 + beta1[2]*x2 + beta1[3]*A + 
                                     beta1[4]*D1 + beta1[5]*D2 + beta1[6]*D3,lin_pred)) %>%
  dplyr::mutate(lin_pred = if_else(latent == 2, group_intercepts_2[level] + beta2[1]*x1 + beta2[2]*x2 + beta2[3]*A + 
                                     beta2[4]*D1 + beta2[5]*D2 + beta2[6]*D3, lin_pred)) %>%
  dplyr::mutate(lin_pred = if_else(latent == 3,
                                   group_intercepts_3[level] +  beta3[1]*x1 + beta3[2]*x2 + beta3[3]*A + 
                                     beta3[4]*D1 + beta3[5]*D2 + beta3[6]*D3, lin_pred ))
...
formula <- y ~ x1 + x2 + A  + D1 + D2 + D3 + (1|level) - 1

...
p_real_clust <- cluster_plot_byclust(sim_data[[2]], sim_data[[2]]$latent,
                                     add_to_title = " (Real clusters)")
p_fitted_clust <- cluster_plot(sim_data[[2]], fit_list_full[[C_real]][[2]],
...
p1 <- MLCWMd_comparison(full = miss_full, partial = miss_partial, 
                        x_breaks = seq(0.0, 1, 0.01), title = "Percentage of Misclassified Observations", 
                        y_title = "Percentage (%)", col_x_axis = F, show.outliers = F)

p2 <- MLCWMd_comparison(full = ari_full, partial = ari_partial, 
                        x_breaks = seq(0.0, 1, 0.02), title = "Adjusted Rand Index Comparison", 
...
p_beta <- beta_plot(data = beta_data, comp_data = comp_data, n_sim = dim(beta_data)[1], 
                lwd = 0.6, C = 3, vars = c("1", "2", "3", "4", "5",
                                         "6", "7"), lty = "dashed", 
                v_just_ann = -0.2, h_just_ann = -0.7, rm.outliers = FALSE)
beta_data <- extract_beta_from_sim_data(fit_list = fit_list_partial, 
                                        sim_data_list = sim_data, C = 3)
p_beta_partial <- beta_plot(data = beta_data, comp_data = comp_data, 
                  n_sim = dim(beta_data)[1], lwd = 0.6, C = 3, 
                  vars = c("1", "2", "3", "4", "5", "6", "7"), lty = "dashed", 
                    v_just_ann = -0.2, h_just_ann = -0.7, rm.outliers = FALSE)
p_beta
...
p_sigma <- sigma_plot(data = sigma_data, n_sim = dim(sigma_data)[1], lwd = 0.8, 
      C = 3, vars = c("1", "2"), lty = "dashed", v_just_ann = -0.2, h_just_ann = -0.7)
...
lam_data <- extract_lambda_from_sim_data(fit_list = fit_list_full, 
            sim_data_list = sim_data, C = 3, v = 1)
p_lam <- lambda_plot(data = lam_data, n_sim = dim(lam_data)[1], 
      lwd = 0.8, C = 3, v = 1, p = 3, lty = "dashed", v_just_ann = -0.2, h_just_ann = -0.7)
...
p_inter <- inter_plot(data = int_data, n_sim = dim(int_data)[1], lwd = 1, 
                 C = 3, vars = c("1", "2", "3"),
                 lty = "dashed", v_just_ann = -0.2, h_just_ann = -0.7)
p_inter
...
p_thres <- thres_plot(data = thres_data, n_sim = dim(thres_data)[1], 
              lwd = 1, C = 3, vars = c("1", "2", "3"), lty = "dashed",
               v_just_ann = -0.2, h_just_ann = -0.7)
p_thres

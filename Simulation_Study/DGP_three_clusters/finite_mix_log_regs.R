...
fit_fm <- flexmix(yy ~ x1 + x2 + factor(V1) + factor(D1) + factor(D2) + factor(D3), 
                  data = sim_data[[1]], k = 3, model = FLXMRglm(family = "binomial"))
...
for(i in 1:K){
  ...
  init_clusters <- kmeans(sim_data[[i]][, c("x1", "x2")], centers = 3)$cluster
  ...
  fit_fm[[i]] <- flexmix(yy ~ x1 + x2 + factor(V1) + factor(D1) + factor(D2) + factor(D3), 
                         data = sim_data[[i]], k = 3, model = FLXMRglm(family = "binomial"),
...
fit <- logisregmixEM(y = sim_data[[1]]$y, x = design_matrix, k = 3,
                     addintercept = TRUE)
